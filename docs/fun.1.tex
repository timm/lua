...
Operators: \passthrough{\lstinline!+= -= *= /=!}. \textbf{LHS may be a
dotted name} (\passthrough{\lstinline![\%w\_\%.]+!} --- so
\passthrough{\lstinline!obj.field += 1!} works) and \textbf{RHS must be
one whitespace-separated token} (\passthrough{\lstinline!\%S+!}).
Bracket-indexed LHS like \passthrough{\lstinline!t[i] += 1!} does
\textbf{not} transform.
...
transpiler uses Lua's balanced-bracket matcher
(\passthrough{\lstinline!\%b[]!}) so nested
\passthrough{\lstinline![...]!} inside the expression, iterator, or
guard is fine
(e.g.~\passthrough{\lstinline![r[i.at] for r in rows if r[i.at] \~= "?"]!}
works), but the opening \passthrough{\lstinline![!} and matching
\passthrough{\lstinline!]!} must be on the \textbf{same physical line}.
...
\subsection{\texorpdfstring{Lua statement-call ambiguity (\texttt{{[}}
or \texttt{(} after a function
call)}{Lua statement-call ambiguity ({[} or ( after a function call)}}\label{lua-statement-call-ambiguity-or-after-a-function-call}

Lua grammar treats \passthrough{\lstinline!f()[k]!} as ``call
\passthrough{\lstinline!f!}, then index its return value at
\passthrough{\lstinline![k]!}'' --- even when the
\passthrough{\lstinline![!} is on the next line. Newlines are not
statement terminators in Lua. So a comprehension or parenthesized
expression on the line \textbf{after} a function call gets glued to that
call:
...
Lua tries to index \passthrough{\lstinline!print!}'s return value (nil)
and errors with \passthrough{\lstinline!attempt to index a nil value!}.
The same trap fires when a following line starts with
\passthrough{\lstinline!(!}:
...
This is a Lua quirk, not a \emph{fun} quirk --- the transpiler emits Lua
exactly as written. Idiomatic rule: prepend \passthrough{\lstinline!;!}
whenever a line starts with \passthrough{\lstinline!(!} or
\passthrough{\lstinline![!} after a function call.
