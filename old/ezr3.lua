...
-- Forward declarations for localization and inter-visibility.
...
-- Cols is a factor that turns a list of strings into specific column roles.
-- [A-Z] names become Nums; others Syms. Ends in [-+!] are goals (y).
...
function Cols(ss_names,    xs,ys,all,col)
  ...
  for n, s in ipairs(ss_names) do
    ...
    if not s:match"X$" then push(s:match"[%+%-!]$" and ys or xs, col) end end
...
-- sub is add with weight -1. adds handles collections.
...
-- Centroids computed once; only re-calculated if state changed.
...
function DATA.mid(i)
  i._mid = i._mid or map(i.cols.all, function(col) return col:mid() end); return i._mid end
...
-- ## Distances
-- mink: Minkowski distance for aggregate goal error calculation.
-- disty: measures regret as normalized distance to ideal targets.
-- wins: normalized distance between population median and best-case results.

function mink(vs,    err,n)
  err, n = 0, 0; for _, x in ipairs(vs) do n=n+1; err=err+abs(x)^the.p end
...
-- ## Tree
-- Explainable model: recursively partitions data to minimize variance.
-- Partitions rows by searching x-features for best variance-reducing split.
-- Leaves summarize target goals (mids) for terminal populations.

function TREE.build(i,data,rows,    mid,best,bestW,w)
  mid, i.y = data:clone(rows):mid(), adds(map(rows, function(r) return i.score(r) end))
  ...
  for _, col in ipairs(data.cols.x) do
    for _, cut in ipairs(col:splits(rows, i.score)) do
      ...
      if w < bestW and min(#cut.left,#cut.right) >= the.leaf then best, bestW = cut, w end 
    end end
  if best then
...
-- ## Splits
-- Divides rows based on a pivot; NUM uses median, SYM uses unique values.

function split(col,rows,fn_score,cut,fn_test,    lhs,rhs,L,R,ok)
  lhs, rhs, L, R = Num(), Num(), {}, {}
...
function NUM.splits(i,rows,fn_score,    vs,n_med,cut)
  ...
  if #vs<2 then return {} end; sort(vs); n_med=vs[floor(#vs/2)+1]
...
function SYM.splits(i,rows,fn_score,    seen,outs,cut)
  ...
  for _, row in ipairs(rows) do
    ...
    if v~="?" and not seen[v] then
      ...
      if cut then push(outs, cut) end 
    end end; return outs end

-- ## Stats
-- Non-parametric comparisons: Cliff's Delta (effect) and KS-test (significance).

function same(xs,ys,eps,    n,m,n_gt,n_lt,ks,fn)
...
function bestRanks(dict,    items,k0,vs0,best)
  items = {}
  for name, vs in pairs(dict) do sort(vs); push(items, {name, vs, vs[floor(#vs/2)+1]}) end
  sort(items, function(a,b) return a[3] < b[3] end); k0, vs0 = items[1][1], items[1][2]
...
eg["--data"] = function (f,    data)
  data = Data(f); for _, c in ipairs(data.cols.y) do
    print(string.format("%s: n=%d mid=%s spread=%s",c.txt,c.n,rat(c:mid()),rat(c:spread()))) 
  end end

eg["--tree"] = function (f,    data,rows)
  data = Data(f); rows = l.many(data.rows, the.Budget); data = data:clone(rows)
  Tree(function(r) return data:disty(r) end):build(data, data.rows):show() end
...
eg["--ranks"] = function (    dict,name,k,len)
  ...
  print("\nTop Tier Treatments:"); for k, num in pairs(bestRanks(dict)) do
    print(string.format("  %-5s median: %s", k, rat(num:mid()))) end end

eg["--test"] = function (f,    data,outs,fn_win,n,test,data2,node,top)
  data = Data(f); outs = Num("win"); fn_win = wins(data)
  for _ = 1, 20 do
    l.shuffle(data.rows); n = #data.rows // 2; test = l.slice(data.rows, n+1)
    data2 = data:clone(l.slice(data.rows, 1, min(n, the.Budget)))
    node = Tree(function(r) return data2:disty(r) end):build(data2, data2.rows)
    sort(test, function(a,b) return node:leaf(a).y:mid() < node:leaf(b).y:mid() end)
    top = sort(l.slice(test, 1, the.Check), function(a,b) 
      return data2:disty(a) < data2:disty(b) end)
    add(outs, fn_win(top[1])) end; print(rat(floor(outs:mid()))) end

-- ## Main
-- CLI entry point: parses help string via SSOT for defaults.
...
local function main(    k,v,n)
  n=1; while n <= #arg do
    ...
    else for k1 in pairs(the) do
      if k == "-"..k1:sub(1,1) then the[k1] = l.thing(v); n=n+1 end end end end end 

for k,v in help:gmatch("([%w_]+)%s*=%s*([^%s]+)") do the[k] = l.thing(v) end
math.randomseed(the.seed); if (arg[0] or ""):match("ezr.*%.lua$") then main() end

return NUM,SYM,COLS,DATA,TREE,Tree,Sym,Num,Cols,Data,add,sub,
       adds,mink,wins,split,same,bestRanks
